...
@Component({
  selector: 'app-add-rigs',
  templateUrl: './add-rigs.component.html',
  styleUrls: ['./add-rigs.component.scss']
})
export class AddRigsComponent {
  RigForm!: FormGroup;
  ErrorMessage = '';
  // json_data: any[] = [];
  UserJsonString:any
  ...
  submitData() {
    if (this.RigForm.valid) {
      this.addDataService.AddRig(this.RigForm.value).subscribe({
        next: data => {
