...
@Component({
  selector: 'app-classification-of-accident',
  templateUrl: './classification-of-accident.component.html',
  styleUrls: ['./classification-of-accident.component.scss']
})
export class ClassificationOfAccidentComponent {
  private ListOfClassificationOfAccident = new BehaviorSubject<IClassificationOfAccident[]>([]);
  readonly ClassificationOfAccidentList = this.ListOfClassificationOfAccident.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteClassificationOfAccident(ClassificationOfAccident: IClassificationOfAccident) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteClassificationOfAccident(ClassificationOfAccident).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetClassificationOfAccidentByPage(num).subscribe({
      next: data => {
        this.ListOfClassificationOfAccident.next(data.items)
