...
export class ClassificationsComponent {
  private ListOfclassification = new BehaviorSubject<IClassification[]>([]);
  readonly classificationList = this.ListOfclassification.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteClassification(Classification: IClassification) {
    if (confirm("Are you sure you want to delete this Record?")) {
      ...
      this.deleteService.DeleteClassification(Classification).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetClassificationByPage(num).subscribe({
      next: data => {
        this.ListOfclassification.next(data.items)
