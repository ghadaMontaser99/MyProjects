...
export class ClientComponent {
  private ListOfClient = new BehaviorSubject<IClient[]>([]);
  readonly ClientList = this.ListOfClient.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteClient(Client: IClient) {
    if (confirm("Are you sure you want to delete this Record?")) {
      ...
      this.deleteService.DeleteClient(Client).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetClientByPage(num).subscribe({
      next: data => {
        this.ListOfClient.next(data.items)
