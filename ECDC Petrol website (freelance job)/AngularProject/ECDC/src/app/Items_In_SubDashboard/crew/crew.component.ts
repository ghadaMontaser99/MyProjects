...
export class CrewComponent {
  private ListOfCrew = new BehaviorSubject<ICrew[]>([]);
  readonly CrewList = this.ListOfCrew.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteCrew(Crew: ICrew) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteCrew(Crew).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetCrewByPage(num).subscribe({
      next: data => {
        this.ListOfCrew.next(data.items)
