...
export class PositionComponent {
  private ListOfPosition = new BehaviorSubject<IPosition[]>([]);
  readonly PositionList = this.ListOfPosition.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeletePosition(Position: IPosition) {
    if (confirm("Are you sure you want to delete this Record?")) {
      ...
      this.deleteService.DeletePosition(Position).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetPositionByPage(num).subscribe({
      next: data => {
        this.ListOfPosition.next(data.items)
