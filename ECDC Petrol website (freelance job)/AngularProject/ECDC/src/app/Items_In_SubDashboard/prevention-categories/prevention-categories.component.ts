...
@Component({
  selector: 'app-prevention-categories',
  templateUrl: './prevention-categories.component.html',
  styleUrls: ['./prevention-categories.component.scss']
})
export class PreventionCategoriesComponent {
  private ListOfPreventionCategory = new BehaviorSubject<IPreventionCategory[]>([]);
  readonly PreventionCategoryList = this.ListOfPreventionCategory.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeletePreventionCategory(PreventionCategory: IPreventionCategory) {
    if (confirm("Are you sure you want to delete this product?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeletePreventionCategory(PreventionCategory).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetPreventionCategoryByPage(num).subscribe({
      next: data => {
        this.ListOfPreventionCategory.next(data.items)
