...
@Component({
  selector: 'app-qhsepositions',
  templateUrl: './qhsepositions.component.html',
  styleUrls: ['./qhsepositions.component.scss']
})
export class QHSEPositionsComponent {
  private ListOfQHSEPosition = new BehaviorSubject<IQHSEPosition[]>([]);
  readonly QHSEPositionList = this.ListOfQHSEPosition.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteQHSEPosition(QHSEPosition: IQHSEPosition) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteQHSEPosition(QHSEPosition).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetQHSEPositionByPage(num).subscribe({
      next: data => {
        this.ListOfQHSEPosition.next(data.items)
