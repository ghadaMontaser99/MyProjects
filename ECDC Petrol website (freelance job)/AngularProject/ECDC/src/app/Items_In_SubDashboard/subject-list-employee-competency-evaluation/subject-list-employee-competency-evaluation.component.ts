...
@Component({
  selector: 'app-subject-list-employee-competency-evaluation',
  templateUrl: './subject-list-employee-competency-evaluation.component.html',
  styleUrls: ['./subject-list-employee-competency-evaluation.component.scss']
})
export class SubjectListEmployeeCompetencyEvaluationComponent {


  private ListOfSubjectList = new BehaviorSubject<ISubjectList[]>([]);
  readonly SubjectList = this.ListOfSubjectList.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteSubjectList(SubjectList: ISubjectList) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteSubjectList(SubjectList).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetSubjectListByPage(num).subscribe({
      next: data => {
        this.ListOfSubjectList.next(data.items)
