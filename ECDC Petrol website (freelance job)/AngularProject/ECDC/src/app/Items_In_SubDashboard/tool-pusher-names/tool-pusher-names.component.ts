...
@Component({
  selector: 'app-tool-pusher-names',
  templateUrl: './tool-pusher-names.component.html',
  styleUrls: ['./tool-pusher-names.component.scss']
})
export class ToolPusherNamesComponent {
  private ListOfToolPusherPositionName = new BehaviorSubject<IToolPusherPositionName[]>([]);
  readonly ToolPusherPositionNameList = this.ListOfToolPusherPositionName.asObservable();
  page:number=1;
  count:number=0;
  ...
  ngOnInit(): void {
    ...
    this.loginService.isAdmin.subscribe({
     ...
     this.dataService.GetToolPusherPositionNameForExcel().subscribe({
      next: data => this.json_data = data.data,
      error: err => console.log(err)
    })
  }

  DeleteToolPusherPositionName(ToolPusherPositionName: IToolPusherPositionName) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteToolPusherPositionName(ToolPusherPositionName).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetToolPusherPositionNameByPage(num).subscribe({
      next: data => {
        this.ListOfToolPusherPositionName.next(data.items)
  ...
  Download() {
    ...
    let worksheet = workbook.addWorksheet("Tool Pusher Names Data");
    ...
    let fname = "Tool Pusher Names Report"
