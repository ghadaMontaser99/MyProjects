...
@Component({
  selector: 'app-typeof-injuries',
  templateUrl: './typeof-injuries.component.html',
  styleUrls: ['./typeof-injuries.component.scss']
})
export class TypeofInjuriesComponent {
  private ListOfTypeOfInjuryList = new BehaviorSubject<ITypeOfInjury[]>([]);
  readonly TypeOfInjuryList = this.ListOfTypeOfInjuryList.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteTypeOfInjury(TypeOfInjury: ITypeOfInjury) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteTypeOfInjury(TypeOfInjury).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetTypeOfInjuryByPage(num).subscribe({
      next: data => {
        this.ListOfTypeOfInjuryList.next(data.items)
