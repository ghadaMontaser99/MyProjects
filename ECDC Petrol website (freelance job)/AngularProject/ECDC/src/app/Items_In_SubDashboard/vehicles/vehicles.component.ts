...
export class VehiclesComponent {
  private ListOfVehicle = new BehaviorSubject<IVehicle[]>([]);
  readonly VehicleList = this.ListOfVehicle.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteVehicle(Vehicle: IVehicle) {
    if (confirm("Are you sure you want to delete this Record?")) {
      ...
      this.deleteService.DeleteVehicles(Vehicle).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetVehicleByPage(num).subscribe({
      next: data => {
        this.ListOfVehicle.next(data.items)
