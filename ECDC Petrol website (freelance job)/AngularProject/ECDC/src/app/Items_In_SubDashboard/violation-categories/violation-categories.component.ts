...
@Component({
  selector: 'app-violation-categories',
  templateUrl: './violation-categories.component.html',
  styleUrls: ['./violation-categories.component.scss']
})
export class ViolationCategoriesComponent {
  private ListOfViolationCateory = new BehaviorSubject<IViolationCategory[]>([]);
  readonly ViolationCategoryList = this.ListOfViolationCateory.asObservable();
  page:number=1;
  count:number=0;
  ...
  DeleteViolationCategory(ViolationCateory: IViolationCategory) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.deleteService.DeleteViolationcCategories(ViolationCateory).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.getDataByPage.GetViolationCategoryByPage(num).subscribe({
      next: data => {
        this.ListOfViolationCateory.next(data.items)
