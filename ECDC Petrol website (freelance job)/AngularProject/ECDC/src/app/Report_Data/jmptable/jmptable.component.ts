...
@Component({
  selector: 'app-jmptable',
  templateUrl: './jmptable.component.html',
  styleUrls: ['./jmptable.component.scss']
})
export class JMPTableComponent {
  ErrorMessage = '';
  private ListOfJMP = new BehaviorSubject<any[]>([]);
  readonly JMPList = this.ListOfJMP.asObservable();
  page: number = 1;
  count: number = 0;
  ...
  constructor(private jmpservice: AddnewJMPService,
    private getDataByPage: GetdataByPageService, private dataService: DataService, private deleteService: DeleteDataService, private loginService: LoginService) { }
  ...
  DeleteJMP(JMP: IJMP) {
    if (confirm("Are you sure you want to delete this Record?")) {
      ...
      this.deleteService.DeleteJMP(JMP).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num: number) {
    this.getDataByPage.GetJMPByPage(num).subscribe({
      next: data => {
        this.ListOfJMP.next(data.items)
