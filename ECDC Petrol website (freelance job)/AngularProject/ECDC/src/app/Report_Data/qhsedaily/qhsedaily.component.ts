...
@Component({
  selector: 'app-qhsedaily',
  templateUrl: './qhsedaily.component.html',
  styleUrls: ['./qhsedaily.component.scss']
})
export class QHSEDailyComponent {
  ErrorMessage = '';
  private ListOfQHSEDaily = new BehaviorSubject<any[]>([]);
  readonly QHSEDailyList = this.ListOfQHSEDaily.asObservable();
  page:number=1;
  count:number=0;
  ...
  constructor(private AddQHSEDaily: AddQHSEDailyService,private getDataByPage:GetdataByPageService,private dataService: DataService, private deleteService: DeleteDataService,private loginService:LoginService) { }
  ...
  ngOnInit() {
    ...
    this.loginService.isAdmin.subscribe({
     ...
     this.AddQHSEDaily.GetQHSEDailys(this.User.ID,this.User.Role).subscribe({
      next: data => this.json_data = data.data,
      error: err => this.ErrorMessage = err
    })
  }

  DeleteQHSEDaily(QHSEDaily: IQHSEDaily) {
    if (confirm("Are you sure you want to delete this Record?")) {
      // user clicked "Yes"
      // do something, such as call an API to delete the item
      this.AddQHSEDaily.DeleteQHSEDaily(QHSEDaily).subscribe({
        next: data => {
          console.log(data)
  ...
  getpages(num:number)
  {
    this.AddQHSEDaily.GetQHSEDailyByPage(num,this.User.ID,this.User.Role).subscribe({
      next: data => {
        this.ListOfQHSEDaily.next(data.items)
  ...
  Download() {
    ...
    let worksheet = workbook.addWorksheet("QHSEDaily Data");
