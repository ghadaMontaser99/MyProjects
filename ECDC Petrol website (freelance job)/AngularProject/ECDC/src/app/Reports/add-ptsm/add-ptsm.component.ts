...
@Component({
  selector: 'app-add-ptsm',
  templateUrl: './add-ptsm.component.html',
  styleUrls: ['./add-ptsm.component.scss']
})
export class AddPTSMComponent {
  PTSMForm!: FormGroup;
  ErrorMessage = '';
  UserJsonString:any
  UserJsonObj:any
  ...
  constructor(private dataService: DataService,private loginService:LoginService,private addPTSMService:PTSMService,private fb: FormBuilder, private router: Router) {
  ...
  submitData() {
    if (this.PTSMForm.valid) {
      this.addPTSMService.AddPTSM(this.PTSMForm.value).subscribe({
        next: data => {
